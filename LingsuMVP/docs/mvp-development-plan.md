...
- Combat stability review fixes: the HTML5 battle prototype now resolves support ultimates by effect fields, heals from each target max HP, applies DEF and timed DEF break in one shared damage path, expires control/attack-break states by target action, counts kills across waves, and registers one resize listener. Unity DropSystem now checks the material-count UI reference before requesting its Canvas so missing presentation wiring cannot block Boss victory settlement.
- Combat stability test: run `node --test tests/playable-battle.test.js`; then open `playable/spirit-codex.html`, verify 潮汐 and 晨曦 ultimates heal/shield without damaging enemies, a one-turn freeze skips exactly one enemy action, high-DEF targets take less damage, and the final result reports 7 kills. In Unity, clear DropSystem `materialCountText`, enter Play Mode, defeat a normal monster and Boss, and confirm no exception occurs and victory still opens.
